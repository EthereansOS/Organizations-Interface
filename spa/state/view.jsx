var State = React.createClass({
    ...
    onInputRef(ref) {
        ...
        if(!ref || !this.addNewDataType) {
            return;
        }

        if(this.addNewDataType.value === 'bool') {
            return this.addNewInput.hide();
        }
        this.addNewDataType.value === 'uint256' && this.addNewInput.attr('type', 'number');
        this.addNewDataType.value === 'uint256' && this.addNewInput.attr('min', '0');
    },
    onRadioRef(ref) {
        this.addNewRadio = $(ref).show();
        if(!ref || !this.addNewDataType || this.addNewDataType.value !== 'bool') {
    ...
    addNew(e) {
        ...
        var dataType = this.addNewDataType.value;
        var inputValue = this.addNewInput.val();
        var checkedValue = this.addNewRadio.children().find('input[type="radio"]:checked').val() === 'true';
        this.controller.addNew(name, dataType, dataType === 'bool' ? checkedValue : inputValue).catch(error => _this.emit('message', error.message || error, 'error'));
    },
    renderStateElement(it, i) {
        ...
        return(<section className="DFOSingleState" data-i={i}>
            <div className="DFOSingleStateTN">
                ...
                <aside className="DFOSingleStateT">&#128190; {it.dataType}</aside>
            </div>
            ...
            <h6 className="DFOSingleStateV">
                {(!_this.props.edit || !_this.state || (_this.state.change !== it.name && !_this.state.changeAll)) && <span>{it.value}</span>}
                {_this.props.edit && _this.state && (_this.state.change === it.name || _this.state.changeAll) && it.dataType !== 'bool' && <input type={it.dataType === 'uint256' ? 'number' : 'text'} min="0" ref={ref => ref && (ref.value = it.value)}/>}
                {_this.props.edit && _this.state && (_this.state.change === it.name || _this.state.changeAll) && it.dataType === 'bool' && <form>
                    <label>
    ...
    render() {
        ...
        {/* _this.props.edit && <a className={"LinkVisualButton LinkVisualButtonB" + (_this.state && _this.state.changeAll ? " Editing" : "")} href="javascript:;" onClick={() => _this.setState({ changeAll: _this.state && _this.state.changeAll ? null : true, change: null })}>Change All</a> }
                ...
                {_this.props.edit && (!_this.state || (!_this.state.changeAll && !_this.state.change)) && <section>
                    ...
                    <select onChange={_this.onTypeChange} ref={ref => _this.addNewDataType = ref}>
