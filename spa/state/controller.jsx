var StateController = function (view) {
    ...
    context.load = function load(view) {
        view = view || context.view;
        view.setState({ stateElements: null, stateElementsAmount: null }, async function () {
            try {
                var stateElements = [];
                view.setState({ stateElements, stateElementsAmount: parseInt(await window.blockchainCall(view.props.element.stateHolder.methods.getStateSize)) });
                ...
                for (var i in json) {
                    ...
                    view.setState({ stateElements });
                }
            } catch (e) {
                view.setState({unsupportedRnDVersion : true});
            }
        });
    };
    ...
    context.proposeStateElement = async function proposeStateElement(element, value, cumulative) {
        ...
        window.sendGeneratedProposal(view.props.element, { title: "Proposal for " + description.firstLetterToLowerCase() }, window.context.stateHolderProposalTemplate, [line], [description]);
    };
    ...
    context.changeAll = async function changeAll(elements) {
        ...
        window.sendGeneratedProposal(view.props.element, { title: descriptions.length > 1 ? "StateHolder change" : ("Proposal for " + descriptions[0].firstLetterToLowerCase()) }, window.context.stateHolderProposalTemplate, lines, descriptions);
    };
    ...
    context.addNew = async function addNew(name, type, value) {
        ...
        for (var i in view.state.stateElements) {
            if (name === view.state.stateElements[i].name) {
