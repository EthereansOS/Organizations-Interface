...
window.keccak256 = require('@ethersproject/solidity');
window.bytecodeHash = '96e8ac4277198ff8b6f785478aa9a39f403cb768dd02cbee326c3e7da348845f';
...
window.getLogs = async function(a, endOnFirstResult) {
    ...
    while (true) {
        ...
        if (parseInt(args.fromBlock) < 0 || parseInt(args.fromBlock) >= parseInt(args.toBlock)) {
    ...
    return new Promise(function(ok, ko) {
        setTimeout(async function() {
            try {
                ...
                } catch (e) {}!window.walletAddress && window.localStorage.removeItem("selectedEthereumProvider");
...
window.getNetworkElement = function getNetworkElement(element, networkId) {
    var network = window.context.ethereumNetwork[networkId || window.networkId || ""];
    if (network === undefined || network === null) {
        return;
    }
...
window.loadTokenInfos = async function loadTokenInfos(addresses, abi, noLogo) {
    ...
    for (var address of addresses) {
        ...
        tokens.push(window.tokenInfosCache[address] = window.tokenInfosCache[address] || {
            ...
            logo: noLogo ? undefined : await window.loadLogo(address)
        });
    }
...
window.loadLogo = async function loadLogo(address) {
    address = window.web3.utils.toChecksumAddress(address);
    var logo = address === window.voidEthereumAddress || address === window.wethAddress ? window.context.ethereumLogoURI : window.context.trustwalletImgURLTemplate.format(address);
    try {
        await window.AJAXRequest(logo);
    } catch (e) {
        logo = window.context.defaultLogoURI;
    }
    return logo;
};
...
window.loadUniswapPairs = async function loadUniswapPairs(address, indexes) {
    window.pairCreatedTopic = window.pairCreatedTopic || window.web3.utils.sha3('PairCreated(address,address,address,uint256)');
    var myToken = address instanceof Array ? address.map(it => window.web3.eth.abi.encodeParameter('address', it)) : [window.web3.eth.abi.encodeParameter('address', address)];
    var logs = await window.getLogs({
        address: window.context.uniSwapV2FactoryAddress,
        fromBlock: window.getNetworkElement('uniswapStartBlock'),
        topics: [
            window.pairCreatedTopic, myToken, myToken
        ]
    });
    var uniswapPairs = [];
    var alreadyAdded = {};
